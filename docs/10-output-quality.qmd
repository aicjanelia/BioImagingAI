...
Recall that, this requires selecting a loss function: a function that represents how far the model's output is from your desired ground truth. 
...
The model's parameters are optimized to minimize the loss on a training dataset, and you evaluate the same loss on a held-out validation dataset to get an idea of how well your model would perform on new data.
...
def make_missing_cells(label_image, interior_fraction=1/6, n_false_positives=3, fp_radius=18, seed=42):
    """Remove all border cells and a random fraction of interior cells,
    then seed a few false-positive detections in the background."""
    ...
    # Add false positives: larger irregular blobs in the background
    next_lbl = label_image.max() + 1
    margin = fp_radius + 2
    valid = np.argwhere(
        (out == 0) &
    ...
    for cy, cx in positions:
        # Random ellipse with varying radii and rotation
...
#|   - "False positives, false negatives, and split nuclei."
...
In this case an appropriate metric would be  pixel-overlap metrics like Intersection over Union (IoU) or the Dice coefficient. 
...
> **Note:** The DICE coefficient is the F1-score applied to pixel-level segmentation masks, and the two names are sometimes used interchangeably in the biomedical imaging literature.
...
In this case, a metric built around per-instance detection, such as precision and recall (or F1-score) after matching predicted masks to ground truth, captures the problems that matter to the analysis. It will also reveal the false positives and false negatives that would need correction.
