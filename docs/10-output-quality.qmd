...
#| fig-cap: An example of tracking solutions with common failure modes.
#| warnings: true
#| echo: false
...
fig, ax = plt.subplots(1, 3, sharex=True, sharey=True, figsize=(7, 3.4))
panels = [
    (plot_gt, "Ground Truth"),
    (plot_no_edge, "Sparse Edge\nSolution"),
    (plot_no_div, "No Division\nSolution"),
]
for axis, (g, title) in zip(ax, panels):
    plot_graph(g, axis)
...
ax[0].set_ylim(11.2, -1.2)  # inverted y (frame increases downward)
ax[0].set_xlim(-1.0, 4.0)
fig.tight_layout(w_pad=0.4)
```

[TODO - consider embedding metric results into the figure]
...
# Remove every third edge from the graph
no_edge = gt.copy()
sparsify_edges(no_edge)

# Create a matching between nodes in the gt and solution
matched_no_edge = Matched(
    TrackingGraph(gt.copy(), frame_key='frame'),
    TrackingGraph(no_edge, frame_key='frame'),
    mapping=[(nid, nid) for nid in gt.nodes],
    matcher_info={}
)
res_no_edge = {
    **CTCMetrics().compute(matched_no_edge).results,
    **DivisionMetrics().compute(matched_no_edge).results['Frame Buffer 0'],
    **BasicMetrics().compute(matched_no_edge).results
}
metrics = [
    'TRA',
...
pprint({
    k: res_no_edge[k] for k in metrics
})
```
...
matched_no_div = Matched(
    TrackingGraph(gt.copy(), frame_key='frame'),
    TrackingGraph(no_divs, frame_key='frame'),
    mapping=[(nid, nid) for nid in gt.nodes],
    matcher_info={}
)
res_no_div = {
    **CTCMetrics().compute(matched_no_div).results,
    **DivisionMetrics().compute(matched_no_div).results['Frame Buffer 0']
}
pprint(res_no_div)
```
