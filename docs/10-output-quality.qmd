...
Recall that, this requires selecting a [loss function](glossary.qmd#loss-function): a function that represents how far the model's output is from your desired ground truth. 
As training relies on [gradient descent](glossary.qmd#gradient-descent), the loss must be differentiable; the algorithm needs the slope of the loss with respect to every parameter to know which direction to step.
The model's parameters are optimized to minimize the loss on a [training dataset](glossary.qmd#training-data), and you evaluate the same loss on a held-out [validation dataset](glossary.qmd#validation-data) to get an idea of how well your model would perform on new data.
...
A [metric](glossary.qmd#quality-control-metric) is any function that scores your predictions.
...
Two standard losses for this task are [binary cross entropy (BCE)](glossary.qmd#binary-cross-entropy), and [mean squared error (MSE)](glossary.qmd#mean-squared-error).
These functions are often optimized for the task of [semantic segmentation](glossary.qmd#semantic-segmentation), separating foreground from background.
...
Binary cross-entropy penalizes the model based on how confident it is in the wrong answer. 
This loss requires the output to be interpretable as a probability — it asks "is this pixel foreground or background?" and treats all wrongly-classified pixels the same way, whether they are at a cell boundary, inside a hole, or belong to an entirely missing cell.
For ground truth label $y_i \in \{0,1\}$ and predicted probability $\hat{y}_i \in [0,1]$:
...
The mean squared error penalizes the model proportionally to the squared difference between its prediction and the ground truth:
$$\text{MSE} = \frac{1}{N} \sum_{i=1}^{N} (y_i - \hat{y}_i)^2$$
MSE measures *how far* the prediction is from the target value, and does not require the output to be a probability — it works equally well when the model produces a continuous map such as a [distance transform](glossary.qmd#distance-transform) or a [confidence score](glossary.qmd#confidence-score).
...
If we are interested in measuring the size of each nucleus, a model which under ([@fig-segmentations]a) or over segments nuclei ([@fig-segmentations]b) would create erroneous measurements of size. 
Instead a model that misses some detections ([@fig-segmentations]c) might be preferable as long as the detections that exist correctly reflect the size of the nucleus. 
In this case an appropriate metric would be  pixel-overlap metrics like [Intersection over Union (IoU)](glossary.qmd#iou) or the [Dice coefficient](glossary.qmd#f1-score). 
A metric that captures this will look very different from BCE or MSE.

:::{.callout-note appearance="simple" icon=false collapse="false"}
# IoU and DICE are overlap metrics.
...
DICE is the [harmonic mean](glossary.qmd#harmonic-mean) of [precision](glossary.qmd#precision) and [recall](glossary.qmd#recall): it balances [false positives](glossary.qmd#false-positives) (predicted pixels outside $A$) and [false negatives](glossary.qmd#false-negatives) (pixels in $A$ missed by $B$) equally. IoU is slightly stricter, because every falsely predicted pixel inflates the union without affecting the sum of the two areas. In practice the two metrics rank models in the same order.
...
Alternatively, if we are interested in counting the number of nuclei, a model which changes the size of the nuclei should not impact the analysis.
However a model that misses nuclei ([@fig-segmentations]c), detects artifacts as nuclei ([@fig-segmentations]d), merges multiple nuclei into one ([@fig-segmentations]b), or splits a single nucleus into multiple ([@fig-segmentations]c) would introduce errors into the final counts. 
In this case, a metric built around per-instance detection, such as [precision](glossary.qmd#precision) and [recall](glossary.qmd#recall) (or [F1-score](glossary.qmd#f1-score)) after matching predicted masks to [ground truth](glossary.qmd#ground-truth), captures the problems that matter to the analysis. It will also reveal the false positives and false negatives that would need correction.
Different downstream uses call for different metrics.

::: {.callout-note appearance="simple" icon=false collapse="false"}
# Precision, recall, and F1-score are instance-level metrics.
...
Open the callouts below for more metrics that are common in image analysis.
...
The [Hausdorff distance](glossary.qmd#hausdorff-distance) measures how far apart two sets of boundary points are at their worst-case point.
...
[Average Precision (AP)](glossary.qmd#average-precision) summarizes the precision–recall trade-off across confidence thresholds.
...
::: {.callout-tip}
Explore this [demo](https://metrics-reloaded.dkfz.de/metric-demo/segmentation) to see the direct impact of different segmentation errors on a variety of metrics.
:::

As you've seen, there are many different metrics available for every task.
...
Cell tracking is typically approached as a multi-stage task. The first stage is detection where cells in each frame of the time series are detected using [point detections](glossary.qmd#centroid-coordinates), [bounding boxes](glossary.qmd#bounding-region) or [segmentation masks](glossary.qmd#binary-segmentation). In the second stage, cells are connected between frames. For the purposes of evaluation, it is helpful to consider a tracking solution as a graph where each cell in a timepoint is a node and edges connect the cells (or nodes) across frames (@fig-tracking-graph). A cell dividing into two daughters appears as a node with two outgoing edges, so a tracking solution is naturally represented as a set of lineage trees. Metrics that evaluate tracking solutions can consider both the performance of the detection phase (the nodes) and the performance of the linking (the edges). 
