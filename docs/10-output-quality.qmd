...
<!-- Basic segmentation label figure -->
```{python}
#| code-fold: true 
#| label: fig-image
...
In @sec-9, you've learned about how to train a machine learning model.
...
<!-- Segmentation error types figure -->
```{python}
#| code-fold: true
...
Consider the case of nuclear segmentation. If we are interested in counting the number of nuclei, a model which under ([@fig-segmentations]a) or over segments a nucleus ([@fig-segmentations]b) and therefore changes the size of the nucleus will not impact the analysis. However a model that misses nuclei ([@fig-segmentations]c), merges multiple nuclei into one ([@fig-segmentations]b), or splits a single nucleus into multiple (Fig TODO) would introduce errors into the final counts. In this case, a metric built around per-instance detection, such as precision and recall (or F1-score) after matching predicted masks to ground truth, captures the problems that matter to the analysis. It will also reveal the false positives and false negatives would need correction.

Alternatively if we are interested in measuring the size of each nucleus, the first model described which under or over segments nuclei would create erroneous measurements of size. Instead a model that misses some detections might be preferable as long as the detections that exist correctly reflect the size of the nucleus. In this case a more appropriate metric would be  pixel-overlap metrics like Intersection over Union (IoU) or the Dice coefficient. 
...
```{python}
# TODO DIANE Can this plot be centered?
import matplotlib.pyplot as plt
...
Conclusion: (Repeats, should also be in the text elsewhere)
- TODO: You need ANNOTATED DATA in order to compute metrics -- they are computed wrt a ground truth
- TODO: Metrics don't replace looking at your data
- OK: Look at more than one metric

Discussion: (More open, can be mentioned here for the first time)
- DONE: Some metrics (e.g. ones that require matching) may be too computationally intensive to compute during training on validation
- DONE: Separation between the metrics you compute on validation (to choose your model/checkpoint) and the metrics you compute on tests (to understand the limitations of your pipeline)
- Segue into uncertainty: some metrics will give caveats about what the model can and cannot do. Uncertainty would be nice to have. Still an unsolved problem. Briefly touch on test-time augmentation in the callout below.
...
- - Validation metrics
  - Fast
  - Val
  - Additional metrics can provide more insight into training progress outside the loss function and assist in checkpoint selection
  - MSE
...
[TODO] Diane, check if this needs to be modified
