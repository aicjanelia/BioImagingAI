...
Consider the case of nuclear segmentation. If we are interested in counting the number of nuclei, a model which under or over segments a nuclei and therefor changes the size of the nucleus will not impact the analysis. However a model that misses nuclei, merges multiple nuclei into one, or splits a single nucleus into multiple would introduce errors into the final counts. In this case, a metric built around per-instance detection, such as precision and recall (or F1-score) after matching predicted masks to ground truth, captures the problems that matter to the analysis. It will also reveal the the false positives and false negatives would need correction.
