...
# Architectures and Loss Models {#sec-4}
...
In the case of biological problems, however, it is often necessary for the output to be the same size as the input -- pixel-level labels -- rather than a single object category label. For example, for cell classification, each pixel in the output may represent the cell class of the pixel. A common architecture for this is a u-net [@ronneberger2015u], which applies filters and downsamples like Alexnet, and then additionally applies filters with upsampling, in the end producing an output at the same pixel resolution as the input.
